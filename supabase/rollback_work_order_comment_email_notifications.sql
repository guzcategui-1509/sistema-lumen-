-- Restore the exact comment producers that existed before unified participant emails.
-- Existing comments, mentions, activities and email rows are preserved.
...
returns table (
  comment_id uuid, work_order_id uuid, author_user_id uuid, parent_comment_id uuid,
  message text, comment_type text, requires_response boolean, resolution_status text,
  resolved_by uuid, resolved_at timestamptz, created_at timestamptz,
  updated_at timestamptz, created_mention_user_ids uuid[]
)
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  select order_row.brand_id, order_row.id, mention.mentioned_user_id,
...
begin
  ...
  if cleaned_body = '' then raise exception 'empty_phase_comment'; end if;
  if char_length(cleaned_body) > 2000 then raise exception 'phase_comment_too_long'; end if;

  select * into phase_row
  from public.work_order_phases where id = target_phase_id;
  if not found then raise exception 'work_order_phase_not_found'; end if;

  select orders.brand_id, orders.archived_at into phase_brand_id, order_archived_at
  from public.work_orders orders where orders.id = phase_row.work_order_id;
  ...
  if not (public.can_manage_work_orders() or phase_row.assigned_to = auth.uid()) then
    raise exception 'not_allowed_to_comment_phase';
  end if;

  insert into public.work_order_phase_comments(work_order_id, phase_id, author_id, body)
  values (phase_row.work_order_id, phase_row.id, auth.uid(), cleaned_body)
  returning * into comment_row;

  insert into public.work_order_activity(work_order_id, actor_id, action, details)
  values (phase_row.work_order_id, auth.uid(), 'phase_comment_added', jsonb_build_object(
    'phase_id', phase_row.id, 'phase_key', phase_row.phase_key, 'title', phase_row.title
  ));
  return comment_row;
end;
$function$;
...
drop function if exists public.render_work_order_comment_notification_email(
  text,text,text,text,text,timestamptz,text,text
);
drop function if exists public.resolve_work_order_notification_recipients(uuid);
